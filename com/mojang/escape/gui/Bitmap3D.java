...
public class Bitmap3D extends Bitmap {
		...
		for (int zb = zCenter - r; zb <= zCenter + r; zb++) {
			for (int xb = xCenter - r; xb <= xCenter + r; xb++) {
				...
				if (c instanceof DoorBlock) {
					...
					if (e.solidRender) {
						renderWall(xb + openness, zb + 0.5 - rr, xb, zb + 0.5
								- rr, c.tex, (c.col & 0xfefefe) >> 1, 0,
								openness);
						renderWall(xb, zb + 0.5 + rr, xb + openness, zb + 0.5
								+ rr, c.tex, (c.col & 0xfefefe) >> 1, openness,
								0);
						renderWall(xb + openness, zb + 0.5 + rr, xb + openness,
								zb + 0.5 - rr, c.tex, c.col, 0.5 - rr, 0.5 + rr);
					} else {
						renderWall(xb + 0.5 - rr, zb, xb + 0.5 - rr, zb
								+ openness, c.tex, c.col, openness, 0);
						renderWall(xb + 0.5 + rr, zb + openness, xb + 0.5 + rr,
								zb, c.tex, c.col, 0, openness);
						renderWall(xb + 0.5 - rr, zb + openness, xb + 0.5 + rr,
								zb + openness, c.tex, (c.col & 0xfefefe) >> 1,
								0.5 - rr, 0.5 + rr);
					}
				...
				if (c.solidRender) {
					...
					if (!s.solidRender) {
						renderWall(xb, zb + 1, xb + 1, zb + 1, c.tex,
								(c.col & 0xfefefe) >> 1);
					}
				} else {
					if (e.solidRender) {
					...
					if (s.solidRender) {
						renderWall(xb + 1, zb + 1, xb, zb + 1, s.tex,
								(s.col & 0xfefefe) >> 1);
					}
				}
			}
			...
			for (int xb = xCenter - r; xb <= xCenter + r; xb++) {
				...
				for (int j = 0; j < c.entities.size(); j++) {
					...
					for (int i = 0; i < e.sprites.size(); i++) {
						...
						renderSprite(e.x + sprite.x, 0 - sprite.y, e.z
								+ sprite.z, sprite.tex, sprite.col);
					}
				}

				for (int i = 0; i < c.sprites.size(); i++) {
					Sprite sprite = c.sprites.get(i);
					renderSprite(xb + sprite.x, 0 - sprite.y, zb + sprite.z,
		...
		for (int y = 0; y < height; y++) {
			...
			for (int x = 0; x < width; x++) {
				...
				if (tex < 0) {
					...
					pixels[x + y * width] = Art.floors.pixels[((xPix & 15) * 4 + (tex % 8) * 64)
							+ ((yPix & 15) * 4 + (tex / 8) * 64) * 512]
							* col;
				}
			}
		}
		...
		for (int yp = yp0; yp < yp1; yp++) {
			...
			for (int xp = xp0; xp < xp1; xp++) {
				...
				if (zBuffer[xp + yp * width] > zz) {
					int col = Art.sprites.pixels[(xt + tex % 8 * 64)
							+ (yt + (tex / 8) * 64) * 512];
					if (col >= 0) {
	...
	private void renderWall(double x0, double y0, double x1, double y1,
			int tex, int color, double xt0, double xt1) {
		...
		for (int x = xp0; x < xp1; x++) {
			...
			for (int y = yp0; y < yp1; y++) {
				...
				int yTex = (int) (64 * pry);
				pixels[x + y * width] = Art.walls.pixels[((xTex) + (tex % 8) * 64)
						+ (yTex + tex / 8 * 64) * 512]
						* color;
	...
	public void postProcess(Level level) {
		for (int i = 0; i < width * height; i++) {
			double zl = zBuffer[i];
			if (zl < 0) {
				int xx = ((int) Math.floor((i % width) - rot * 2048
						/ (Math.PI * 2))) & 2047;
				int yy = i / width;
				pixels[i] = Art.sky.pixels[xx + yy * 2048] * 0x444455;
			} else {
				int xp = (i % width);
				int yp = (i / width) * 14;
